...
export const nhlAuditState: SportAuditDiagnostic = {
  sport: 'NHL',
  sourceName: 'ESPN NHL Public Scoreboard API',
...
function recordError(endpoint: string, message: string) {
  nhlAuditState.sourceStatus = 'ERROR';
  nhlAuditState.recentErrors.unshift({
    timestamp: new Date().toISOString(),
    endpoint,
    message,
  });
  if (nhlAuditState.recentErrors.length > 10) {
    nhlAuditState.recentErrors.pop();
  }
}

export async function fetchNhlSchedule(dateInput?: string): Promise<{
  scheduleDate: string;
  source: 'ESPN';
  ...
  const url = `https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/scoreboard?dates=${espnDateParam}`;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`ESPN NHL API returned HTTP ${response.status}: ${response.statusText}`);
    }

    const data: any = await response.json();
    const lastVerifiedAt = new Date().toISOString();
    const leagueName = data.leagues?.[0]?.abbreviation || data.leagues?.[0]?.name || 'NHL';
    ...
    nhlAuditState.sourceStatus = 'OPERATIONAL';
    nhlAuditState.lastScheduleFetch = {
    ...
    recordError('/api/schedule?sport=NHL', err.message || 'Failed to fetch NHL schedule');
    throw err;
  }
}

export async function fetchNhlLiveScores(): Promise<{
  sport: 'NHL';
  lastVerifiedAt: string;
  count: number;
  liveCount: number;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`ESPN NHL LiveScore HTTP ${response.status}: ${response.statusText}`);
    }
    ...
    nhlAuditState.sourceStatus = 'OPERATIONAL';
    nhlAuditState.lastLiveScoreFetch = {
      timestamp: lastVerifiedAt,
    ...
    recordError('/api/live-scores?sport=NHL', err.message || 'Failed to fetch NHL live scores');
    throw err;
  }
}
